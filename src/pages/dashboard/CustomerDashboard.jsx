...
import { 
  ...
  BadgeDollarSign, LayoutDashboard, Search 
} from 'lucide-react';
...
export default function CustomerDashboard() {
  ...
  const fetchCustomerData = async () => {
    ...
    try {
      ...
      setStats({
        ...
        inProgress: validData.filter(o => ['assigned', 'in_progress'].includes(o.status)).length,
        completed: validData.filter(o => o.status === 'completed').length,
        totalSpent: validData.reduce((s, o) => s + (parseFloat(o.total_amount) || 0), 0)
      });
      setOrders(validData);
    } catch (err) {
  ...
  useEffect(() => {
    if (userRole === 'customer') fetchCustomerData();
  }, [user, userRole]);
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-gray-900 transition-all p-4 md:p-10">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-12">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="text-4xl font-black dark:text-white">Customer Hub</h1>
              <p className="text-gray-500 font-bold uppercase text-xs">My Premium Orders</p>
            </div>
          </div>
          <button onClick={() => navigate('/orders/new')} className="bg-blue-600 hover:bg-blue-700 text-white px-10 py-5 rounded-2xl font-black flex items-center gap-3 shadow-xl transition-all">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-[3rem] shadow-sm border dark:border-gray-700 overflow-hidden">
          <div className="flex p-4 bg-gray-50/50 dark:bg-gray-800/50 border-b dark:border-gray-700 gap-2">
            {['all', 'active', 'completed'].map(t => (
              <button 
                key={t} onClick={() => setActiveTab(t)} 
                className={`px-6 py-2 rounded-xl capitalize font-bold text-sm transition-all ${activeTab === t ? 'bg-white dark:bg-gray-600 shadow-sm text-blue-600 dark:text-white' : 'text-gray-500'}`}
              >
                {t}
              </button>
            ))}
